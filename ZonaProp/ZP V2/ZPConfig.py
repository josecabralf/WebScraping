URL_ZonaProp = 'https://www.zonaprop.com.ar/casas-departamentos-ph-terrenos-venta-cordoba.html'
URL_Base = 'https://www.zonaprop.com.ar'
URL_UltimaSemana = 'https://www.zonaprop.com.ar/casas-departamentos-ph-terrenos-venta-cordoba-publicado-hace-menos-de-1-semana.html'

archivos_ZonaProp = './Casas/'
revisionesFecha = './Revisiones/revisionesFecha.txt'
fechasRevisadas = './Revisiones/fechas.txt'
