...
def crearListaLinks(URL):
    ...
    pagina_casas = soup.find_all('div', class_="sc-i1odl-0 crUUno") # Obtenemos todas las casas de la página
    
    links_casas = set()
    for link in pagina_casas:
        links_casas.add(link["data-to-posting"])
...
def escribirArchivo(archivo, links_casas):
    ...
    with open(archivo, 'w', encoding='utf-8') as archivoJSON:
        ...
        for casa in links_casas:
            objetoJSON = scrapZonaPropPublicacion(URL_Base + casa) # Scrap de la publicación de la casa
